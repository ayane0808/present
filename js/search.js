...
function extractBudgetRange(text) {
  const matches = [...String(text || '').matchAll(/(\d[\d,]*)\s*円/g)].map((m) =>
...
function buildPostContext(posts) {
  ...
  return posts
    .map((post, idx) => {
      ...
      return `${idx + 1}. 商品: ${post.productName} / 価格: ${price} / カテゴリ: ${post.category || '不明'} / シーン: ${post.scene || '不明'} / 関係: ${post.relation || '不明'} / URL: ${url} / 口コミ: ${post.review || 'なし'}`;
    })
    .join('\n');
}
...
function buildDbSectionForUser(posts, userMsg, conditions) {
  ...
  const lines = posts.slice(0, 3).map((post) => {
    ...
    return `- ${post.productName}${pricePart}がおすすめです。\n  理由: ${reasonText}。\n  口コミ: ${shortReview}${post.review && post.review.length > 42 ? '...' : ''}${urlPart}`;
  });
